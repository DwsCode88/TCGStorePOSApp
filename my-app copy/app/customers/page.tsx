...
export default function LabelsPage() {
  ...
  const generateBarcodeValue = (item: InventoryItem) => {
    // For consignment items, use customer's vendor code
    if (item.acquisitionType === "consignment" && item.customerVendorCode) {
      return `${item.customerVendorCode}-${item.id}`;
    }
    // For non-consignment or consignment without customer code, use store vendor code
    return vendorCode ? `${vendorCode}-${item.id}` : item.id;
  };
